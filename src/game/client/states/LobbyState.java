...
/**
 * Create the lobby state for the game, where players choose their characters
 * and can see other players that have connected to their game. This also deals
 * with the transition to the actual game.
 * 
 * @author SabreWulf
 *
 */
public class LobbyState extends AbstractState {
	...
	/**
	 * Create a new lobby for the given application
	 * 
	 * @param app
	 */
	...
	/**
	 * Initialise the lobby state by setting up sound, textures and buttons.
	 * Also communicates with the server once lock in has been pressed so that
	 * other players can be notified.
	 */
	@Override
	public void init() {
		super.init();
		...
		Texture lobbyBackgroundTexture = app.getAssetManager()
				.getTexture("res/textures/gui/backgrounds/lobby_background.png");
		...
		Texture lockInPressedTexture = app.getAssetManager()
				.getTexture("res/textures/gui/buttons/lock_button_pressed.png");
		Texture lockInReleasedTexture = app.getAssetManager()
				.getTexture("res/textures/gui/buttons/lock_button_released.png");
		...
		Texture quitPressedTexture = app.getAssetManager()
				.getTexture("res/textures/gui/buttons/quit_button_pressed.png");
		Texture quitReleasedTexture = app.getAssetManager()
				.getTexture("res/textures/gui/buttons/quit_button_released.png");
		...
		for (int i = 0; i < 6; i++) {
			...
			if (i < 3) {
				...
				Label label = new Label(3.0f, 35.0f + (i * 24.5f), app.getAssetManager().getFont("fontSprite.png"),
						3.0f, 0.7f, new Vec4(1.0f, 1.0f, 1.0f, 1.0f));
				...
				Label label = new Label(87.5f, 35.0f + ((i % 3) * 24.5f),
						app.getAssetManager().getFont("fontSprite.png"), 3.0f, 0.7f, new Vec4(1.0f, 1.0f, 1.0f, 1.0f));
				playerNames.add(label);
				app.getGui().add(label);
			}
		}
